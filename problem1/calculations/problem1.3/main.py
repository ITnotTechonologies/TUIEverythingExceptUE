...
def cmp_main(angle):
    ...
    t_projectile = s1 / projectile_speed + ((abs(starting_angle - angle)) / gun_rotation_speed)
    s2 = distanceTraveledByTarget(target_speed, t_projectile)
...
for i in range(8):
    ...
    if abs(intersection[0] - target_traveled[0]) <= target_width:
        print("Result: Target can be reached by projectile \n Angle: {}".format(angle))
        possible = True
        break
