...
#     global intersection
#     s1 = distanceTraveledByProjectile(angle, target_position[1])
...
#         print()
#         if abs(intersection[0] - target_traveled[0]) <= (target_width / 4):
#             print("Result: Yes \n Angle: {}".format(angle))
#             possible = True
#             break
...
#             target_movement = 0
#             fps_movement = 0
#             target_position[1] = projectile_rect.centery - target_rect.centery
#             target_position[0] = target_rect.centerx - projectile_rect.centerx
...
#                 else:
#                     v_target = 10
#             if event.ui_element == a_target_entry:
#                 if event.text:
#                      try:
#                          a_target = float(event.text)
#                      except ValueError:
#                          a_target = 1
#                 else:
#                      a_target = 1
#             if event.ui_element == v_projectile_entry:
#                 if event.text:
#                      try:
#                          v_projectile = float(event.text)
#                      except ValueError:
#                          v_projectile = 1
#                 else:
#                      v_projectile = 1
...
# target_rect = target.get_rect(center = (357, 398))
#
# run_variable = True
# action_move = False
# gun_rotation = False
...
# x = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 60), (30, 30)), html_text = 'X:', manager=manager)
# y = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1085, 60), (30, 30)), html_text = 'Y:', manager=manager)
#
# target_positionX_entry = pygame_gui.elements.UITextEntryLine(relative_rect = pygame.Rect((1030, 60), (50, 30)), manager=manager)
# speed_target_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 120), (250, 30)), html_text = 'Speed of a target(entry):', manager=manager)
# v_target_entry = pygame_gui.elements.UITextEntryLine(relative_rect = pygame.Rect((1255, 120), (50, 30)), manager=manager)
# v_projectile_entry = pygame_gui.elements.UITextEntryLine(relative_rect = pygame.Rect((1255, 330), (50, 30)), manager=manager)
# speed_projectile_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 330), (250, 30)), html_text = 'Speed of a projectile(entry):', manager=manager)
#
# a_target_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 180), (260, 60)), html_text = 'Acceleration of a target(entry):', manager=manager)
# a_target_entry = pygame_gui.elements.UITextEntryLine(relative_rect = pygame.Rect((1260, 180), (40, 30)), manager=manager)
# target_positionY_entry = pygame_gui.elements.UITextEntryLine(relative_rect = pygame.Rect((1110, 60), (50, 30)), manager=manager)
# gun_angle_entry = pygame_gui.elements.UITextEntryLine(relative_rect = pygame.Rect((1205, 360), (50, 30)), manager=manager)
# start_button = pygame_gui.elements.UIButton(relative_rect = pygame.Rect((1100, 400), (100, 50)), text = "Start", manager = manager)
# # restart_button = pygame_gui.elements.UIButton(relative_rect = pygame.Rect((1100, 460), (100, 50)), text = "Restart", manager = manager)
#
...
#             action_move = False
#
#         if gun_rotation and fps_movement > 2:
...
#             v_target += a_target
#
#     gun_angle_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 360), (200, 30)), html_text = 'Gun angle: {}'.format(gun_angle), manager=manager)
#     projectile_position_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 240), (300, 60)), html_text = 'Projectile position: \n x: {} y: {}'.format(projectile_rect.centerx, projectile_rect.centery), manager=manager)
#     target_position_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 5), (300, 60)), html_text = 'Target position: \n x: {} y: {}'.format(target_rect.centerx, target_rect.centery), manager=manager)
#     speed_target_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 90), (300, 30)), html_text = 'Speed of a target: {}'.format(v_target), manager=manager)
#     a_target_text_box2 = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 150), (300, 30)), html_text = 'Acceleration of a target: {}'.format(a_target), manager=manager)
#     speed_target_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 300), (300, 30)), html_text = 'Speed of a projectile: {}'.format(v_projectile), manager=manager)
#
...
while run_variable:  # game loop
    ...
    if action_move:
        # fps_movement
        fps_movement += 1
        if fps_movement > ceil(t_projectile) + 1:
            action_move = False
            print("target: ", target_rect.centerx - gun_rect.centerx, gun_rect.centery - target_rect.centery)

            print("projectile: ", projectile_rect.centerx - gun_rect.centerx,
                  gun_rect.centery - projectile_rect.centery)
        ...
        if fps_movement > 0:
            ...
            # manager.update(time_delta)
    gun_angle_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 360), (200, 30)),
                                                       html_text='Gun angle: {}'.format(gun_angle), manager=manager)
    projectile_position_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 240), (300, 60)),
                                                                 html_text='Projectile position: \n x: {} y: {}'.format(
                                                                     projectile_rect.centerx, projectile_rect.centery),
                                                                 manager=manager)
    target_position_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 5), (300, 60)),
                                                             html_text='Target position: \n x: {} y: {}'.format(
                                                                 target_rect.centerx, target_rect.centery),
                                                             manager=manager)
    speed_target_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 90), (300, 30)),
                                                          html_text='Speed of a target: {}'.format(v_target),
                                                          manager=manager)
    a_target_text_box2 = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 150), (300, 30)),
                                                       html_text='Acceleration of a target: {}'.format(a_target),
                                                       manager=manager)
    speed_target_text_box = pygame_gui.elements.UITextBox(relative_rect=pygame.Rect((1005, 300), (300, 30)),
                                                          html_text='Speed of a projectile: {}'.format(v_projectile),
                                                          manager=manager)
